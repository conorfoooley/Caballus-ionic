...
import { UserService, RoleService, UserHorseRelationshipService } from '@nst-caballus/dal';
...
export class JwtStrategy extends PassportStrategy(Strategy) {
    constructor(
        ...
        private readonly userHorseRelationshipService: UserHorseRelationshipService
    ) {
        super({
    ...
    public async validate(payload: JwtPayload): Promise<User> {
        ...
        switch (payload.tokenType) {
            case JwtTokenType.PasswordReset:
                ...
                break;
            case JwtTokenType.VerifyEmail:
                user.permissions = [Permission.VerifyEmail];
                break;
            case JwtTokenType.Subscription:
                user.permissions = [Permission.Subscription];
                break;
            ...
            default:
                ...
                const relations = await this.userHorseRelationshipService.getUserHorseRelationshipByUserId(
                    user._id
                );
                user.horseRelationships = !!relations ? relations.summaries : [];
                break;
        }
        return user;
    }
